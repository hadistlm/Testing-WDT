<template>
  <div class="h-screen flex">
    <div class="hidden lg:flex w-full lg:w-1/2 login_img_section justify-around items-center">
      <div class="bg-black opacity-20 inset-0 z-0" />
      <div class="w-full mx-auto px-20 flex-col items-center space-y-6">
        <h1 class="text-white font-bold text-4xl font-sans">
          Testing WDT
        </h1>
        <p class="text-white mt-1">
          WDT Testing Blog CMS
        </p>
      </div>
    </div>
    <div class="flex w-full lg:w-1/2 justify-center items-center bg-white space-y-8">
      <div class="w-full px-8 md:px-32 lg:px-24">
        <form class="bg-white rounded-md shadow-2xl p-5" @submit.prevent="submit">
          <h1 class="text-gray-800 font-bold text-2xl mb-1">
            Hello Again!
          </h1>
          <p class="text-sm font-normal text-gray-600 mb-8">
            Welcome Back
          </p>
          <div v-show="errmsg" class="flex bg-red-100 rounded-lg p-4 mb-4 text-sm text-red-700" role="alert">
            <svg class="w-5 h-5 inline mr-3" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clip-rule="evenodd"></path></svg>
            <div>
                <span class="font-medium">Failed!</span> {{ errmsg }}
            </div>
          </div>
          <div class="flex items-center border-2 mb-8 py-2 px-3 rounded-2xl">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M16 12a4 4 0 10-8 0 4 4 0 008 0zm0 0v1.5a2.5 2.5 0 005 0V12a9 9 0 10-9 9m4.5-1.206a8.959 8.959 0 01-4.5 1.207" />
            </svg>
            <input
              v-model="email"
              class="pl-2 w-full outline-none border-none"
              type="email"
              name="email"
              placeholder="Email Address"
            />
          </div>
          <div class="flex items-center border-2 mb-12 py-2 px-3 rounded-2xl ">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-gray-400" viewBox="0 0 20 20" fill="currentColor">
              <path fillRule="evenodd" d="M5 9V7a5 5 0 0110 0v2a2 2 0 012 2v5a2 2 0 01-2 2H5a2 2 0 01-2-2v-5a2 2 0 012-2zm8-2v2H7V7a3 3 0 016 0z" clipRule="evenodd" />
            </svg>
            <input
              v-model="password"
              class="pl-2 w-full outline-none border-none"
              type="password"
              name="password"
              placeholder="Password"
            />
          </div>
          <button
            type="submit"
            :disabled="loading || !email || !password"
            :class="{
              'bg-indigo-200': loading || !email || !password,
              'bg-indigo-600 hover:bg-indigo-700': !loading && email && password
            }"
            class="block w-full  mt-5 py-2 rounded-2xl hover:-translate-y-1 transition-all duration-500 text-white font-semibold mb-2"
          >
            Login
          </button>
          <div class="flex justify-between mt-4">
            <NuxtLink
              to="/"
              class="text-sm ml-2 hover:text-blue-500 cursor-pointer"
            >
              Kembali ke home
            </NuxtLink>
          </div>
        </form>
      </div>
    </div>
  </div>
</template>

<script>
export default {
  name: 'LoginPage',
  layout: 'login',
  middleware: 'isLoggedin',
  auth: false,
  data () {
    return {
      email: '',
      password: '',
      rememberMe: '',
      loading: false,
      errmsg: ''
    }
  },
  methods: {
    async submit () {
      // clear cookies first
      this.$cookies.removeAll()
      this.errmsg = ''
      this.loading = true

      try {
        await this.$auth
          .loginWith('local', {
            data: {
              username: this.email,
              password: this.password
            }
          })
          .then(async (res) => {
            const payload = {
              username: res.data.username,
              email: this.email,
              user: res.data.user,
              token: res.data.token,
              token_type: res.data.token_type,
              expired_at: res.data.expired_at
            }
            // save to state
            await this.$auth.setUser(payload)
            await this.$cookies.set('auth._user', payload.user)
            // set user token
            this.$auth.setUserToken(payload.token)
            // set token access
            this.$auth.strategy.token.set(payload.token)
            // save to local storage
            this.$cookies.set('auth', JSON.stringify(payload))
          })
      } catch (err) {
        if (err.response) {
          this.errmsg = err.response.data.title || err.response.data.message
        } else {
          this.errmsg = err.message
        }
      }

      this.loading = false
    }
  }
}
</script>

<style scoped>
  .login_img_section {
    background: linear-gradient(rgba(2,2,2,.7),rgba(0,0,0,.7)),url(https://images.unsplash.com/photo-1650825556125-060e52d40bd0?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1170&q=80) center center;
  }
</style>
